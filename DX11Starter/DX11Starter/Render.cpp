...
Render::Render(ID3D11DeviceContext*	ctx, ID3D11Device* d, float w, float h, ID3D11RenderTargetView* bBRTV, ID3D11DepthStencilView* dSV, IDXGISwapChain* sC)
{
	// Initialize fields
	vertexShader = 0;
	pixelShader = 0;
...
void Render::Update(float deltaTime, float totalTime)
{
...
void Render::RotateCamera(float x, float y)
{
	camera->Rotate(x, y);
}
